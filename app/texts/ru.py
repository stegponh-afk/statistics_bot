...
KEY_HOWTO = (
    ...
    f"{SEPARATOR}\n"
    "Сначала общее для всех способов: добавьте этого бота администратором в ваши "
    "каналы, создайте бота в разделе «Боты» и отметьте в нём каналы («Каналы»).\n\n"
    ...
    "При создании бота вы получили ссылку вида\n"
    "<code>{base}/v1/check/ВАШ_КЛЮЧ?user_id=…</code>\n"
    "В конструкторе добавьте действие «HTTP-запрос» (GET) с этой ссылкой, вместо "
    "<code>…</code> подставьте переменную с ID пользователя. В ответе будет поле "
    ...
    "<code>GET {base}/v1/check?user_id=ID</code> с заголовком "
    "<code>Authorization: Bearer ВАШ_КЛЮЧ</code>. Ответ:\n"
    ...
    "<code>GET {base}/v1/channels</code> — список каналов ключа."
)
...
KEY_HOWTO_USERS = (
    ...
    "Добавьте токен бота (кнопка «Добавить токен»). Все, кого ваш бот проверял через "
    "ссылку выше, автоматически попадают в аудиторию. Остальных можно добавить запросом\n"
